...
### Flujo 1: Agregar productos y ver estadísticas
```graphql
...
# Paso 3: Ver el resumen actualizado
query {
  summary {
    totalItems
    totalCost
...
### Flujo 2: Buscar, actualizar y exportar
```graphql
# Paso 1: Buscar productos de Apple
query {
  wishlist(filterName: "Apple") {
    id
    name
...
# Paso 2: Actualizar el stock de uno
mutation {
  updateItem(
    itemId: "3"
    input: {
...
# Paso 3: Exportar todo a CSV
mutation {
  generateCsv
}
```

### Flujo 3: Navegar por páginas y ordenar
```graphql
# Paso 1: Ver los más caros (primera página)
query {
  wishlist(sortByPrice: DESC, page: 1, limit: 5) {
    name
    price
  }
}

# Paso 2: Ver la siguiente página
query {
  wishlist(sortByPrice: DESC, page: 2, limit: 5) {
    name
    price
  }
}

# Paso 3: Ver los más baratos
query {
  wishlist(sortByPrice: ASC, limit: 5) {
    name
    price
  }
}
